...
Scope `sparklogs.agent.vector` (value) or `sparklogs.agent.log` (value).
...
`app` (LQL) is `sparklogs_agent` (value) (`guides/app-vocabulary.md`).
...
Collection honesty lives on feed reports and `agent_complete_through` (col) (`guides/device-state-fields.md`).
