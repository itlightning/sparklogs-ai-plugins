...
**Accuracy.** Security sign-in events carry `win.eventlog.security.logon_type_name`, the authenticating package, and `sparklogs.actor.*`.
...
Group by `reason`, `win.eventlog.security.logon_type_name`.
Failing-then-retrying looks nothing like a slow single `logon_cached_interactive`.

Decoded auth failures (`win.eventlog.security.status_meaning` is the cause without opening the body):
...
Several users on one site slow, others not: same filter, group by `source`.
