...
| Feed | What it is |
|---|---|
| `win.eventlog.setup` (value) | Setup channel: package target-state failures (`patch_servicing_failed` (value)), component-store scan (`store_corruption` (value)). |
...
Windows Update agent snapshot: `sparklogs.agent.state` (value) topic `windows_update_agent_state` (other). Open `feeds/sparklogs.agent.state/` only after this theme. That is inventory/state, not the Setup channel.
