...
|---|---|---|---|
| `sparklogs.data.top_processes.pid` | integer |  | The process id; with `create_time_raw` it is this row's identity, the same one the processes topic uses. 0 on the remainder row. |
| `sparklogs.data.top_processes.create_time_raw` | integer |  | The process's creation time as the raw Windows `FILETIME` value. The Unix epoch on the remainder row. |
| `sparklogs.data.top_processes.image_name` | string |  | The process's executable file name, or `(other processes)` on the remainder row. |
| `sparklogs.data.top_processes.services` | string_array |  | The services this process hosts, sorted. Absent when it hosts none. |
| `sparklogs.data.top_processes.is_remainder` | bool |  | True on the one row summing every process the rankings did not name, so the rows add up to what the processes used (a process that exited inside the window loses its last interval, and interrupt and DPC time belong to no process). |
| `sparklogs.data.top_processes.top_by` | string_array |  | Which rankings named this process: `cpu`, `ram`, `io_read`, `io_write`. Empty on the remainder row. |
