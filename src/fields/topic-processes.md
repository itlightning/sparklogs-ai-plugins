...
| `sparklogs.data.processes.cpu_busy_pct_avg` | float | percent | CPU this process used since the oldest retained baseline (an enumeration kept every 5 minutes, at most an hour old), as a percent of the whole machine: every logical core, so a process saturating one core of eight reads 12.5. Present only on a process that was already running at that baseline, same pid and creation time; a process younger than the baseline (started within the last hour, or since the agent started watching) has no interval to measure and carries none of the three averages. Absent on the System Idle Process (pid 0), whose CPU time is the machine's idle time. |
