...
| `sparklogs.data.agent_overhead.working_set_avg_mb` | float | megabytes | This process's mean resident working set over the window. |
...
| `sparklogs.data.agent_overhead.combined_working_set_avg_mb` | float | megabytes | Sum of the agent and Vector mean working sets, reported on the agent row and used to assess the RAM budget. |
