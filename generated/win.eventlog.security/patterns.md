...
Enumerating only what can co-occur has the opposite failure: the judgment about what is physically possible is made once, by hand, and every honest new combination reads as drift.
...
An unexpected pattern is one of three things, in falling order of likelihood: a new curated surface nobody regenerated this file for, a vocabulary that gained a value, or a token rendered from something that is not a closed vocabulary at all. The third is the one that matters.
...
Two surfaces below render the same head and headline, so a pattern matching it is attributed to whichever of them accepts its tokens.
