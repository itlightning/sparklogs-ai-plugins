...
Every field named below is checked against the generated field schema when this file is built, so a recipe that names a field the pack no longer writes fails the build rather than rotting here.
...
3. Attribute what is left to the calling process. One dominant path (a backup or management agent) confirms noise; a spread of paths does not.

   ```
   sparklogs.reason = "logon_failed" AND sparklogs.process.path != null
   ```
...
4. Check whether it landed: a successful account sign-in from the same address inside the window.
...
2. Read win.eventlog.security.caller_computer off the result. That names the machine, in the provider form (synthetic example: WKSTN-042).

3. Pull that machine's failures for the same account.
...
1. The account and platform arms of privilege assignment are separate curated shapes, so the principal kind selects between them.
   ...
   ```
   event_id = 4672 AND sparklogs.actor.kind = "account"
   ```
...
4. Drop self-refresh, where the presented account is the caller's own.

   ```
   sparklogs.reason = "explicit_credential_use"
     AND sparklogs.actor.name != sparklogs.target.name
   ```
