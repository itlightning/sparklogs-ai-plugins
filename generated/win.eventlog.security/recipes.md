...
These recipes are linted when this file is built, and the lint checks exactly this much: module-prefixed and portable field references resolve against the generated field schema; raw provider references require a module with a named payload; and inside query strings, with quoted literals masked out first, bare field names must be platform standard fields, vendor-envelope references are admitted by namespace only, null predicates in either spelling (`= null`, `IS NULL`) are rejected (presence is the `field!` form), a namespaced field on the right-hand side of a comparison is rejected (a comparison takes a literal there), and a field-positioned token with an uppercase letter is rejected (LQL field references are lowercase).
Anything outside that list is not machine-checked, so read prose claims as prose.
...
A failure whose code is not in the decode table leaves the cause unset and renders no cause token. A rising count of those is a curation gap, not noise.
