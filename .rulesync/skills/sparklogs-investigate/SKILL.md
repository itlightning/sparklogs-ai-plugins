...
| `query_event_counts_by_severity` | backing scan | Group every matching event by the `group_by` fields: one ranks that field's values, 2-3 cross-tab. Top values by hit count. The workhorse for "what's happening" within an LQL filter - run it BEFORE raw logs. |
