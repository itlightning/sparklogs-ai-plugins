...
- Use a direct keyed lookup **only when the instance key is already known**. Do not discover instance maps via `list_fields` on `sparklogs.agent.state` (see `stream-kinds/device-state.md`).
