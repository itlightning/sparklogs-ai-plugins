...
Agent Plugins v1 does **not** expand placeholders in remote URLs or HTTP headers. `${SPARKLOGS_API_TOKEN}` is therefore literal text, not a variable reference. Edit `mcp.json` and replace the whole value of the `Authorization` key under `mcpServers.sparklogs.headers` with `Bearer` followed by your token.
