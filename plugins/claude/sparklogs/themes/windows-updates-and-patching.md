...
Explore Setup vs CBS/DISM vs state: `${CLAUDE_PLUGIN_ROOT}/guides/stream-kinds.md`.
...
**Pivots.** Consecutive failed cycles beat a single Setup/3. Read `${CLAUDE_PLUGIN_ROOT}/feeds/<id>/reasons.md` for the slug, then counts grouped by `sparklogs.reason` (LQL). HRESULT lives on Setup fields (`${CLAUDE_PLUGIN_ROOT}/feeds/win.eventlog.setup/fields.md`).
