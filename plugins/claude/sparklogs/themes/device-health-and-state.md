...
Explore this feed as device state, not WEL: `${CLAUDE_PLUGIN_ROOT}/guides/stream-kinds/device-state.md`.
...
**Honesty (supporting).** `agent_complete_through` (col) and collection liveness decide what you may say about a gap. Open monitor ≠ problem (`${CLAUDE_PLUGIN_ROOT}/guides/category-classes.md`). Duration and clear time: `${CLAUDE_PLUGIN_ROOT}/guides/device-state-fields.md` (`episode_age_basis` (col), `episode_clear_time_basis` (col)).
