...
Empty playbook LQL is a miss on the recipe: widen by `subsource` (LQL), then that kind's explore ladder (`${CLAUDE_PLUGIN_ROOT}/guides/stream-kinds.md`), then raw logs. Do not close with "cannot analyze" while that is untried.
Detail: `${CLAUDE_PLUGIN_ROOT}/playbooks/playbooks.md`.
...
`subsource` (LQL) is the directory name. Kind (how to explore, including WEL `provider_name` (LQL) vs device-state maps): `${CLAUDE_PLUGIN_ROOT}/guides/stream-kinds.md`. Then `${CLAUDE_PLUGIN_ROOT}/feeds/<id>/README.md`, then **one** of fields / enums / reasons (Security also recipes / patterns / mappings). Search `reasons.md` for the `##` heading that matches the reason slug; do not read the whole file.
...
Every investigation produces a structured document in this order. The full template lives in `references/output-template.md` with field definitions and worked examples. Write every free-text field per `${CLAUDE_PLUGIN_ROOT}/guides/writing-voice.md`. The structure here is the minimum.
...
The complete per-investigation-type list is in `${CLAUDE_PLUGIN_ROOT}/guides/off-endpoint-causes.md`. Read that file when investigating any specific symptom and customize the WHAT WAS NOT CHECKED section to the actual investigation scope.
...
The full per-tool decision tree is in `${CLAUDE_PLUGIN_ROOT}/guides/mcp-tool-decision-tree.md`. The full per-investigation-type playbook outlines are in `${CLAUDE_PLUGIN_ROOT}/playbooks/playbooks.md`.
...
Full detail and a worked localize-then-land shape: `${CLAUDE_PLUGIN_ROOT}/guides/scope-ladder.md`. The controlled `service` (LQL) vocabulary (the cross-vendor ticket-class values worth pivoting on, e.g. `backup`, `storage`, `security_audit`) is in `${CLAUDE_PLUGIN_ROOT}/guides/service-taxonomy.md`.
...
**Scope resolution sequence - see `${CLAUDE_PLUGIN_ROOT}/guides/scope-resolution.md` for details.** In brief:
...
**Critical+ fetch-first rule:** any non-zero `cnt_critical_plus` (col) in scope (severity 20 and above) means fetch those events before proceeding, regardless of the investigation topic. Critical+ admissions are rare, always-surface facts (confirmed integrity loss or compromise) and auto-elevate into daily fleet reporting; never leave one unread in a Finding's scope. The Info..Error bands carry no fetch-first mandate - weigh them normally. See `${CLAUDE_PLUGIN_ROOT}/guides/category-classes.md`, Query notes.
...
**Sender-first LQL:** filter with `agent_id = "<uuid>"` for everything one sender shipped; use `source` (LQL) for origin-host pivots. "Collector" means one thing only: the log-shipping process the agent supervises on the device. See `${CLAUDE_PLUGIN_ROOT}/guides/scope-resolution.md`.
...
Detailed per-tool usage with examples is in `${CLAUDE_PLUGIN_ROOT}/guides/mcp-tool-decision-tree.md`.
...
The complete LQL reference with all operators, edge cases, and common mistakes is in `${CLAUDE_PLUGIN_ROOT}/guides/lql-reference.md`.
...
Bulk extractive summarization suits the fastest lightweight model tier your host offers; you stay on the more capable model for cross-correlating inference, hypothesis evaluation, and template assembly. Definitions and host-specific notes are in `${CLAUDE_PLUGIN_ROOT}/guides/subagent-definitions.md`.
...
The full list of common mistakes, anti-patterns, and recovery is in `${CLAUDE_PLUGIN_ROOT}/guides/common-mistakes.md`. Top items:
...
- `${CLAUDE_PLUGIN_ROOT}/guides/scope-ladder.md` - the six grouping fields and their `_hash` companions (incl. `source` (LQL)/`source_hash` (LQL)), availability, `query_scope_activity` vs `query_event_counts_by_severity`, and RCA usage shapes.
- `${CLAUDE_PLUGIN_ROOT}/guides/category-classes.md` - what NOTABLE / ELEVATED / RECOVERED mean in `category` (LQL) (temporal shape, not importance), **open monitor ≠ problem**, the lifecycle pair convention, how "interesting" counts fold them in, and the critical+ fetch-first contract.
- `${CLAUDE_PLUGIN_ROOT}/guides/service-taxonomy.md` - the controlled `service` (LQL) ticket-class vocabulary (cross-vendor pivot values), the audit-adjacent demarcation list (why `security_audit` is not the whole audit surface), and boundary rules.
- `${CLAUDE_PLUGIN_ROOT}/playbooks/backup-failure.md` (and siblings in the Section 3b table) - incomplete starter for one symptom. Do not load all playbooks. Empty recipe LQL is not the end of the investigation.
- `${CLAUDE_PLUGIN_ROOT}/themes/windows-security-and-audit.md` - change analysis; other themes in Section 3b.
- `${CLAUDE_PLUGIN_ROOT}/feeds/<id>/` - generated lookup (fields, enums, reasons). Router: Section 3b feed table.
- `${CLAUDE_PLUGIN_ROOT}/guides/device-state-fields.md` - device and agent state: the `query_device_health` surface, the column names, and the honesty fields that decide what you may say about a duration or a clear time.
- `${CLAUDE_PLUGIN_ROOT}/guides/generated-reference-router.md` - how to reach the per-source generated reference set (fields, vocabularies, patterns, recipes) by question shape.
- `${CLAUDE_PLUGIN_ROOT}/guides/scope-resolution.md` - detailed scope-resolution and source-discovery sequence.
- `${CLAUDE_PLUGIN_ROOT}/guides/lql-reference.md` - complete LQL syntax reference with examples and common mistakes.
- `${CLAUDE_PLUGIN_ROOT}/guides/mcp-tool-decision-tree.md` - per-tool detailed usage, all parameters, decision tree for which tool to use when.
- `${CLAUDE_PLUGIN_ROOT}/guides/off-endpoint-causes.md` - per-investigation-type lists of what's not checked and why.
- `${CLAUDE_PLUGIN_ROOT}/guides/common-mistakes.md` - anti-pattern catalog with examples and recoveries.
- `${CLAUDE_PLUGIN_ROOT}/guides/msp-tool-registry.md` - common MSP tools with category/log-location/source-field mappings.
- `${CLAUDE_PLUGIN_ROOT}/guides/pattern-catalog.md` - high-signal `pattern_hash` (LQL) patterns with likely meanings.
- `${CLAUDE_PLUGIN_ROOT}/guides/subagent-definitions.md` - pre-configured subagent definitions for bulk-summarization delegation.
- `${CLAUDE_PLUGIN_ROOT}/guides/writing-voice.md` - style rules for every free-text field you write.
