...
| `(other)` | Syntax / pedagogy (`in`, `null`, `asc`) | stripped on render | not in any product set; not dotted |
...
Dual names: LQL is `sparklogs.reason` (LQL); device-health column is the same spelling, `sparklogs.reason` (col); filter argument is `reasons` (arg).
Future tools (`query_device_state`) are not `(tool)` until they ship.
