...
`playbooks/`, `themes/`, `../feeds/<id>/` (`README.md`, `reasons.md`, `enums.md`, `fields.md`, `recipes.md`, `patterns.md` where present), `guides/`. Artifact choice detail: `generated-reference-router.md`.

## After you pick a `subsource` (LQL)

1. Open `../feeds/<id>/README.md` (short index).
2. **Stream kind** and explore ladder: `stream-kinds.md`. Classic WEL: `provider_name` (LQL) before `pattern` (LQL); device state: `query_device_health` with `sparklogs.kind` (LQL) / `sparklogs.topic` (LQL) / `sparklogs.reason` (LQL).
