...
Until `query_device_state` exists, inventory and snapshot fieldsets on `query_device_health` carry the device state (performance, storage_io, disk_volumes, processes, services, agent_self_resource, pending_reboot, installed products, crash dump config, VSS writers, the Windows Update topic).
