...
// Sync generated AI references from a sibling source-library checkout into src/feeds/ and src/fields/.
//
...
//
// Usage:
//   node scripts/sync-generated-references.mjs           write references, router inventory,
...
import { safeRmFeedModule, safeRmFieldReferences } from './safe-rm-generated.mjs';
import {
  ARTIFACT_SUMMARY,
  KNOWN_DEFECTS,
...
async function readFields(libDir) {
  const src = path.join(libDir, LIBRARY_GENERATED_SUBPATH, FIELD_SOURCE_DIR);
  if (!await exists(src)) throw new Error(`Source library has no generated field reference: ${src}`);
  const entries = await fs.readdir(src, { withFileTypes: true });
  const bad = entries.filter((entry) => !entry.isFile() || !entry.name.endsWith('.md'));
  if (bad.length > 0) {
    throw new Error(`Field reference contains unsupported entries: ${bad.map((entry) => entry.name).join(', ')}`);
  }
  const names = entries.map((entry) => entry.name).sort();
  if (!names.includes('INDEX.md')) throw new Error('Field reference has no INDEX.md');
  const files = new Map();
  for (const name of names) files.set(name, await fs.readFile(path.join(src, name), 'utf8'));
  return files;
}

function manifestBody(commit, modules, fields) {
  return {
    note: 'Generated by scripts/sync-generated-references.mjs. Do not hand-edit src/feeds/ or src/fields/.',
    source: {
    ...
    fields: {
      directory: FIELD_SOURCE_DIR,
      artifacts: [...fields.keys()],
    },
...
function routerInventory(modules, fields) {
  ...
  for (const name of fields.keys()) lines.push(`- \`fields/${name}\``);
  lines.push('');
  return lines.join('\n').trimEnd();
}

async function renderRouter(modules, fields) {
  ...
  return { file, body: `${head}\n\n${routerInventory(modules, fields)}\n\n${tail}`, current: text };
}
...
async function main() {
  ...
  const libraryModules = (await fs.readdir(path.join(dir, LIBRARY_GENERATED_SUBPATH), { withFileTypes: true }))
    .filter((entry) => entry.isDirectory()
      && entry.name !== FIELD_SOURCE_DIR
      && entry.name !== WEBSITE_REFERENCE_DIR)
  ...
  const fields = await readFields(dir);
  const manifest = `${JSON.stringify(manifestBody(commit, modules, fields), null, 2)}\n`;
  const router = await renderRouter(modules, fields);
  ...
  if (CHECK) {
    ...
    const fieldDest = path.join(ROOT, FIELD_GENERATED_DIR);
    const presentFields = await exists(fieldDest) ? (await fs.readdir(fieldDest)).sort() : [];
    for (const [name, body] of fields) {
      const target = path.join(fieldDest, name);
      const current = await exists(target) ? await fs.readFile(target, 'utf8') : null;
      if (current !== body) drifted.push(path.relative(ROOT, target));
    }
    for (const name of presentFields) {
      if (!fields.has(name)) drifted.push(`${path.relative(ROOT, path.join(fieldDest, name))} (not produced by the sync)`);
    }
    ...
    if (drifted.length > 0) {
      throw new Error(`Generated references differ from a re-sync of ${commit.sha}:\n  ${drifted.join('\n  ')}\nRun: yarn sync-generated`);
    }
  ...
  await safeRmFieldReferences();
  const fieldDest = path.join(ROOT, FIELD_GENERATED_DIR);
  await fs.mkdir(fieldDest, { recursive: true });
  for (const [name, body] of fields) await fs.writeFile(path.join(fieldDest, name), body, 'utf8');
