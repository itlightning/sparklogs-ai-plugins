...
- `skills/`: `sparklogs-ask`, `sparklogs-investigate`, `sparklogs-analyze-cause`, `sparklogs-feedback`, in the Agent Skills layout. Each skill carries the reference corpus it cites under its own references directory, so a skill directory is self-contained.
